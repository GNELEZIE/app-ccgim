<?php
if(isset($_SESSION['_ccgim_202']) and isset($_POST['password']) and isset($_POST['passwordC'])  and isset($_POST['passwordA']) and isset($_SESSION['myformkey']) and isset($_POST['formkey']) and $_SESSION['myformkey'] == $_POST['formkey']){
    extract($_POST);
    ...
    $result = $utilisateur->getUtilisateurById($_SESSION['_ccgim_202']['id_utilisateur']);
        ...
        if($password == $passwordC){
            if($data = $result->fetch()){
                if(password_verify($passwordA,$data['mot_de_passe'])){
                    ...
                    $update = $utilisateur->updatePassword($mdpCript,$_SESSION['_ccgim_202']['id_utilisateur']);
                    if($update >0){
